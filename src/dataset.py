...
class BaseDataset(Dataset):
    def __init__(self, behaviors_path, news_path):
        ...
        if not config.training_type.startswith('_impre'):
    ...
    def __getitem__(self, idx):
        ...
        #     news_list = random.sample(row.candidate_news_pb.split(), config.negative_sampling_ratio)
        #     news_list.insert(0,row.candidate_news_pb.split()[0])
        #     # print(news_list)
        #     item["candidate_news"] = [
        #         self.news2dict[x] for x in news_list
        #     ]
        elif config.training_type.startswith('_random'):
            news_list = random.sample(row.candidate_news_random.split(), config.negative_sampling_ratio)
            news_list.insert(0,row.candidate_news_random.split()[0])
            item["candidate_news"] = [
                self.news2dict[x] for x in news_list
            ]           
            # print(news_list) 
