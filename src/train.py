...
def train():
    ...
    for i in tqdm(range(
        ...
        if last_exhaustion_count != exhaustion_count:
            last_exhaustion_count += 1

            (model if model_name != 'Exp1' else models[0]).eval()
            val_auc, val_mrr, val_ndcg5, val_ndcg10 = evaluate(
                model if model_name != 'Exp1' else models[0], f'./data/{dataset_type}/test',
                config.num_workers, 2000000)
            # val_auc, val_mrr, val_ndcg5, val_ndcg10 = evaluate(
            #     model if model_name != 'Exp1' else models[0], f'./data/{dataset_type}/val',
            #     config.num_workers, 2000000)
            (model if model_name != 'Exp1' else models[0]).train()
            ...
            if config.checkpoint_num == '':
                checkpoint_file = f"./checkpoint/{dataset_type}/{model_name}{config.training_type}_{config.negative_sampling_ratio}/ckpt-{step}.pth"
            else:    
                checkpoint_file = f"./checkpoint/{dataset_type}_{config.checkpoint_num}/{model_name}{config.training_type}_{config.negative_sampling_ratio}/ckpt-{step}.pth"
            if early_stop:
...
if __name__ == '__main__':
    ...
    train()
    train_time = int(start - time.time())
