...
def evaluate(model, directory, num_workers, max_count=sys.maxsize):
    ...
    if dataset.startswith(f'_lt{lifetime}_K'):
        ...
        user_dataset = UserDataset(path.join(directory, f'behaviors{dataset}.tsv'),
                                f'data/{dataset_type}/train/user2int.tsv')
        behaviors_dataset = BehaviorsDataset(path.join(directory, f'behaviors{dataset}.tsv'))
...
if __name__ == '__main__':
    ...
    if config.checkpoint_num == '':
        checkpoint_path = latest_checkpoint(path.join(f'./checkpoint/{dataset_type}', f'{model_name}{config.training_type}{config.negative_sampling_ratio}'))
        print(path.join(f'./checkpoint/{dataset_type}', f'{model_name}{config.training_type}_{config.negative_sampling_ratio}'))        
    else:
        ...
        print(path.join(f'./checkpoint/{dataset_type}_{config.checkpoint_num}', f'{model_name}{config.training_type}_{config.negative_sampling_ratio}'))
